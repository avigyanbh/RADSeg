...
This module provides a wrapper around the TextRegion method that generates
text-aligned region tokens using SAM2 masks and frozen image-text models (CLIP/SigLIP)
and outputs segmentation logits in the pixel space.
...
Typical Usage:
  ...
  labels = ["car", "person", "road", "building"]

  enc = TextRegionEncoderConverted(
    device="cuda",
    sam2_checkpoint="path/to/sam2_checkpoint.pth",
    classes=labels
  )
  
  # Returns segmentation logits of shape (B, num_classes+1, H, W)
  seg_logits = enc.encode_image_to_feat_map(rgb_img)
...
class TextRegionEncoder(ImageSemSegEncoder):
    """TextRegion encoder
    """
    ...
    def encode_image_to_feat_map(
        ...
        # region_logits: (R, num_classes) -> (R, num_classes, 1, 1)
        # unique_masks: (R, H, W) -> (R, 1, H, W)
        # seg_logits: (R, num_classes, H, W)
        seg_logits = region_logits.unsqueeze(-1).unsqueeze(-1) * unique_masks.unsqueeze(1)
        
        # Sum across regions: (num_classes, H, W)
        ...
        # Add batch dimension
        seg_logits = seg_logits.unsqueeze(0)  # (1, num_classes+1, H, W)
